...
@section('content')
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单管理 <span class="c-gray en">&gt;</span> 待处理<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="16">待处理列表</th>
        </tr>
        <tr class="text-c">
            <th>订单编号</th>
            <th>订单名称</th>
        ...
        @foreach($data as $value)
            <tr class="text-c">
                ...
                <td>@if($value->status==3)付款成功
                    @elseif($value->status==8)<a href="javascript:addMoney('{{$value->id}}', 1);">申请加价</a>
                    @else用户申请取消订单@endif</td>
                <td>{{$value->createtime}}</td>
                <td>
                    <a title="支付详情" href="javascript:;" onclick="order_pay('支付详情','/admin/order/order/pay','300','400','{{$value->id}}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6b7;</i></a></td>
            </tr>
        @endforeach
        </tbody>
    </table>
