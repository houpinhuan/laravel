            ...
            <div class="formControls col-xs-8 col-sm-9">
                ...
                <span class="select-box">
                    <select class="select" name="mid" size="1">
                        <option value="">请选择</option>
                        @foreach($menu->getList(Auth::guard('admin')->user()->rid) as $value)
                        <option value="{{ $value->id }}" {{old('mid') == $value->id ? 'selected="selected"' : '' }}>{{ $value->name }}</option>
                        @endforeach
                    </select>
                </span>
                <strong style="color: red;">{{$errors->first('mid')}}</strong>
                @else
                <span class="select-box">
                    <select class="select" name="mid" size="1">
                        <option value="">请选择</option>
                        @foreach($menu->getList(Auth::guard('admin')->user()->rid) as $value)
                        <option value="{{ $value->id }}" {{$data->mid == $value->id ? 'selected="selected"' : '' }}>{{ $value->name }}</option>
                        @endforeach
                    </select>
                </span>
                @endif
