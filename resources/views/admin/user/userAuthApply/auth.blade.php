...
<article class="page-container">
    <form method="post" action="">
        {{csrf_field()}}
        <table style="width: 520px;height: 600px">
            <tr>
                <td>真实姓名:</td>
                <td>{{$data->realname}}</td>
            </tr>
            ...
            <tr>
                <td>性别:</td>
                <td>@if($user->gender==1)男@elseif($user->gender==2)女@else人妖@endif</td>
            </tr>
            <tr>
                <td>身份证号码:</td>
                <td>{{$data->cardno}}</td>
            ...
            <tr>
                <td>角色:</td>
                <td>@if($user->role==1)用户@elseif($user->role==2)美发师@else商户@endif</td>
            </tr>
            <tr>
                <td>备注:</td>
                <td>{{$data->remark}}</td>
            ...
            <tr>
                <td>身份证正面照:</td>
                @if(!empty($attach[3]))
                    <td><img src="{{$attach[3]}}" height="50px"></td>
                @else
                    <td>无</td> @endif
            </tr>
            <tr>
            ...
            @if($data->status != 0)
                <tr>
                    ...
                    <td>@if($data->status==1)通过@else驳回@endif</td>
                </tr>
            @else
                <tr>
                    <td style="color: red">审核状态:</td>
